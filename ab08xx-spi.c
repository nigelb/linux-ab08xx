...
static void ab08xx_mark(void)
{
	...
	status = spi_write_then_read(ab08xx_clock.spi, &addr, sizeof(addr),
                        NULL, 0);
	if(status){}
}

static void ab08xx_read_mark(void)
{
	u8 addr[4];
	int status;
	...
	status = spi_write_then_read(ab08xx_clock.spi, &addr, 1,
                        &addr, 4);
...
{
	printk(KERN_EMERG "AB08xx MSB: %i\n", ab08xx_clock.id_msb);
...
static int ab08xx_read_time(struct device *dev, struct rtc_time *dt)
{
	...
	status = spi_write_then_read(ab08xx->spi, &addr, sizeof(addr),
        ...
        dt->tm_mon = bcd2bin(0x1F & buf[count++]) - 1;
...
static int ab08xx_set_time(struct device *dev, struct rtc_time *dt)
{
	...
	buf[count] = ((~0x1F & buf[count++]) | (0x1F & bin2bcd(dt->tm_mon))) + 1;
...
static int ab08xx_set_alarm(struct device *dev, struct rtc_wkalrm *alm)
{
	...
	buf[6] = ((~0x1F & buf[6]) | (0x1F & bin2bcd(dt->tm_mon))) + 1;
...
static int ab08xx_probe(struct spi_device *spi)
{
	...
	printk(KERN_DEBUG "++++++++++++++++++++Starting clock...\n");
	memset(&ab08xx_clock, 0, sizeof(ab08xx_clock));
	printk(KERN_DEBUG "AB08xx MSB: %i\n", ab08xx_clock.id_msb);
